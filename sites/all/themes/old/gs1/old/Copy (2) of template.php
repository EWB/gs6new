    ...
    foreach ($vars['primary_links'] as $index => $link) {
      if ($prepos) { //haven't gotten to the active tab yet
        if ($index != $front_tab) { //haven't gotten to the active tab yet
          $vars['primary_colors'] .= '#navbar-inner li.' . $index 
            . ' {background-image:url(sites/all/themes/gs1/grafix/' . $colors[$index][2] . ");}\n";
        }else{ //at the active tab
	  $vars['primary_colors'] .= '#primary {border-bottom: 36px solid ' . $colors[$index_short][0] . ";}\n"
	    . '#navbar-inner li.' . $index_short
            . ' {background-image:url(sites/all/themes/gs1/grafix/' . $colors[$index_short][2] . ");}\n";
	  $prepos = 0;
        }
      }else{ //after the active tab
	$vars['primary_colors'] .= '#navbar-inner li.' . $index 
          . ' {background-image:url(sites/all/themes/gs1/grafix/' . $colors[$index][1] . ");}\n";
      }
    }
  }
}
